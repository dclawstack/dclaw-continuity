...
# Check events
kubectl get events -n dclaw-continuity --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-continuity deployment/dclaw-continuity-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-continuity-db-credentials -n dclaw-continuity
   ```
