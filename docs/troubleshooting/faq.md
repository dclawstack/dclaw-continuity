...
### How do I update DClaw Continuity?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp continuity --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Continuity without Kubernetes?

Yes, for local development:

```bash
cd dclaw-continuity/frontend && npm run dev
cd dclaw-continuity/backend && uvicorn dclaw_continuity.main:app --reload
```
